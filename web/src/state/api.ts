...
export async function apiFetch(options: LabflowOptions, auth0ClientFn: () => Auth0Client | undefined, method: string, path: string, body?: any): Promise<Response> {
    switch (options.authProvider) {
        case 'auth0':
            ...
            if (auth0Client) {
                ...
                throw "Auth0 is not initialized yet!";
            }
        case 'none':
